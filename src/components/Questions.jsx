  ...
  return (
    <div className="py-10 min-h-screen">
      ...
      {questionAns.map((item,index)=>{
        return(
            <div>
        <div onClick={()=>handleTogle(index)} className="cursor-pointer text-2xl bg-neutral-800 p-5 flex items-center justify-between px-10 mt-3 hover:bg-neutral-700">
          <div className="">{item.question}</div>
          <div className="text-5xl duration-500 ">
            {activeIndex === index ? "x" : "+"}
          </div>
        </div>
        <div className={`bg-neutral-800 overflow-hidden text-2xl  ${activeIndex === index ? "max-h-[500px]" : "max-h-0"} duration-500 px-5 mt-1`}>
          <div className="py-5 px-5">{item.answer}</div>
        </div>
      </div>
        )
      })}
